...
@Component("orcamentoConfeccaoListView")
@Scope("view")
public class OrcamentoConfeccaoListView
		extends AbstractEntityListView<Orcamento, OrcamentoDataMapper, OrcamentoFinder> {
	...
	protected static Logger logger = Logger.getLogger(OrcamentoConfeccaoListView.class);
	...
	public void handleFiltrosPadrao() {
		if (!getFiltros().containsKey("tipoOrcamento") || (getFiltros().get("tipoOrcamento") == null)) {
			getFiltros().put("tipoOrcamento", TipoOrcamento.CONFECCOES);
		}
	}
	...
	protected void handleFilterDatas() {
		...
		getFilterDatas()
				.add(new FilterData("strPesquisa", FilterType.LIKE_ANYWHERE, "cliente.pessoa.nome", "cliente.pessoa.nomeFantasia"));
